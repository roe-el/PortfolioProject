$(document).ready(function() {
    ...
    $(document).on('click', ".jav-tracker", function() {
        $('.output').html("<p>Acted as team leader among four team members. I set up meetings and delegated work while also working alongside my teammates to compelete the project. We developed and presented a solution for tracking the university shuttle buses that run throughout the campus and local community.  I wrote code for the Android app and the Arduino Uno module,  came up with the database solution, tested all elements of the project, and created Power Points for presentations.<p>I also co-wrote and edited the final project report which consisted of images of equipment, tables displaying milestones, flowcharts displaying user interaction with the app and device communication, and detailed information about our research.");
    });
    ...
    $(document).on('click', ".cybrary", function() {
        $('.output').html("<p>Cyber security learning resource that offers video courses on networking, operating systems, virtual machines, professional certifications, and computer security. <p>Courses I have completed cover topics such as subnetting, TCP/IP, network fundamentals, and becoming a MCSA.");
    });
    ...
    $(document).on('click', ".acm", function() {
        $('.output').html("<p>While a member of my universities ACM chapter, I helped write and refactor code which was used to create the chapter <a href='https://www.acm-tamuk.com/' target='_blank' rel='noopener'>website</a>. <p>I also helped plan and participated in social activities to bolter membership and raise money for competitions.");
    });
